...
namespace WebApplication6.Services
{
    public class ServiceWorker(MeisterContext _context, IWebHostEnvironment appEnvironment) : IWorkerService
    {
        ...
        //********************************************************************************
        async Task IWorkerService.Update(UpdateWorker sender)
        {
        ...
        //********************************************************************************
        UpdateWorker IWorkerService.Find(int id)
        {
            UpdateWorker result = null;

            Worker? query = _context.Workers.Find(id);
            if (query != null)
            {
                result = new UpdateWorker()
                {
                    ID = query.Id,
                    FName = query.Fname,
                    LName = query.Lname,
        ...
        //********************************************************************************
        async Task IWorkerService.Insert(InsertWorker sender)
        {
            var wwwroot = appEnvironment.WebRootPath;
        ...
        //********************************************************************************
        IEnumerable<WorkerSelect> IWorkerService.Read()
        {

            List<WorkerSelect> result = new List<WorkerSelect>();
