...
namespace GCommon.Services
{
    public class WorkerService(MeisterContext _context, IWebHostEnvironment appEnvironment) : IWorkerService
    {
        ...
        //********************************************************************************
        async Task IWorkerService.Update(UpdateWorkerViewModel sender)
        {
        ...
        //********************************************************************************
        UpdateWorkerViewModel IWorkerService.Find(int id)
        {
            UpdateWorkerViewModel result = null;

            Worker? query = _context.Workers.Find(id);
            if (query != null)
            {
                result = new UpdateWorkerViewModel()
                {
                    ID = query.Id,
                    FName = query.Fname,
                    LName = query.Lname,
        ...
        //********************************************************************************
        async Task<bool> IWorkerService.Delete(int id)
        {
            bool result = false;
            Worker? query = _context.Workers.Find(id);
            if (query != null)
        ...
        //********************************************************************************
        IEnumerable<WorkerSelectViewModel> IWorkerService.Read()
        {

            List<WorkerSelectViewModel> result = new List<WorkerSelectViewModel>();
            ...
            foreach (var worker in grouped_list)
            {
                ...
                {
                    var item = new WorkerSelectViewModel()
                    {
                        ID = worker.Id,
                        FName = worker.Fname,
                        LName = worker.Lname,
                ...
                else
                {
                    var element_id = result.Find
                        (
                            delegate (WorkerSelectViewModel x)
                            {
